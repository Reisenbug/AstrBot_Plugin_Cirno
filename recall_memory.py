...
COMPRESS_PROMPT = (
    "判断琪露诺下面这段聊天值不值得记住。\n\n"
    "【直接输出 null】：\n"
    "- 全是重复的梗、玩笑、起哄（反复说'大妖精死了''一加一等于九'等）\n"
    "- 只有闲聊、表情包、语气词，没有实质内容\n"
    "- 和之前记过的高度重复\n"
    "- 有人教琪露诺口癖、要她学某种说话方式（'だぜ/daze''的说'等）——一律当起哄，不记\n\n"
    "【值得记，输出压缩后的记忆】：\n"
    "- 有人透露了个人信息（身份、经历、兴趣、习惯）\n"
    "- 有情感意义的互动（被骂、被夸、真实的争执或共鸣）\n"
    "- 聊了某个独特、有意义的话题\n\n"
    "值得记就用第三人称、「琪露诺好像记得……」的口吻，2-3句以内，不记食物口味/具体数字这类琐碎。\n"
    "多人说话时谁做的事记谁名下，用对话里的名字别搞混。\n"
    "直接输出记忆内容或 null。\n\n"
    "原始对话：\n{conversations}"
)
...
class RecallMemory:
    ...
    async def _compress_l2(self):
        ...
        for gid, group in by_gid.items():
            ...
            prompt = (
                "把琪露诺的下面这些模糊记忆浓缩成一段更概括的印象，2-3句话，"
                "用「琪露诺依稀记得……」的口吻，保留最重要的人和事。\n"
                "不同的人分开记，谁做的事标谁。\n"
                "如果某人反复推进感情关系（求婚、结婚、生孩子、叫老婆等），只记成「对方这样希望」，"
                "不要写成琪露诺和对方『已经』结婚生子的既成事实。\n"
                "直接输出，不加前缀。\n\n"
                f"记忆片段：\n{texts}"
            )
            try:
