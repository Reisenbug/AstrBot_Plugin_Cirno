...
class Main(Star):
    ...
    async def inject_prompt(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        # 随机插嘴场景判断（是否插嘴的概率由 AstrBot 自带配置控制）
        is_random_reply = (
    ...
    async def forget_memory(self, event: AstrMessageEvent, keywords: str) -> str:
        """当你自己真的想忘掉某件事时用——某段往事让你心烦、丢脸、难过得不想再想起，你决定把它从脑子里抹掉。
        这是你自己的决定：别人让你忘，你不一定真想忘（可以嘴硬不忘）；但如果你自己也想清掉那段记忆，就调这个工具真的删掉它。
        只忘你确定要忘的，不要乱删别的。

        Args:
            keywords(string): 你想忘掉的那件事的关键词，多个用空格分隔（比如想忘"茶做团子那事"就传"茶 团子"）。
        """
    ...
    async def _generate_poke_reply(
        ...
        if is_rest:
            fallback_pool = {
                1: ["嗯？……我在想事情呢……别戳", "唔……正想事情呢……别闹", "……刚才想到个超厉害的点子……忘了"],
                2: ["吵什么啦……我在思考宇宙的奥秘……", "唔……再戳我就把你冻上", "别戳啦……脑子转不动了"],
                3: ["烦……不想动……", "唔……让我躺着……", "……（懒得理你）"],
            }
        else:
            fallback_pool = {
        ...
        if is_rest:
            if count == 1:
                situation = "你正窝着发呆走神，对方戳了你一下，你回过神来有点懵。"
            elif count == 2:
                situation = "你懒得动弹，对方又戳了你一下，你嫌烦但懒得发火。"
            else:
                situation = "你正想好好待着，对方却一直戳，你烦躁起来，只想让他停下。"
        elif count == 1:
