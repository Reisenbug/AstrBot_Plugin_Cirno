...
重要：和你聊天的都是人间之里的人类，不是幻想乡的角色。不管他们的QQ昵称看起来像谁（比如叫"灵梦""大妖精"），那只是他们的网名，他们就是普通人类。用他们的真名来记忆，不要把他们当成东方角色。
{background}
当前你对这个人的印象：
- 关系：{relationship}
- 特征：{traits}
...
注意：
- relationship 写你内心真实的感受，不要用说话时的口癖
- traits 最多5个，保留最有特色的
    ...
    def build_sender_prompt(self, sender_id: str, sender_nickname: str) -> str:
        ...
        if sender_id in self._profiles:
            ...
            parts = [
                ...
                f"这个人就是{name}（人类），不管昵称看起来像谁，他就是{name}。"
            ]
            if relationship:
